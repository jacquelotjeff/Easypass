...
public class PasswordParamFilter implements Filter {
    ...
    public static final Logger log = Logger.getLogger(PasswordParamFilter.class.getName());
    ...
    private void restrict(HttpSession session, HttpServletResponse response, HttpServletRequest request) throws IOException {
        ...
        if (request.getRequestURI().contains(BackGroupServlet.URL_BASE)) {
            response.sendRedirect(session.getServletContext().getContextPath() + BackPasswordServlet.URL_BASE);
        } else {
            response.sendRedirect(session.getServletContext().getContextPath() + FrontPasswordServlet.URL_BASE);
        }
    }
